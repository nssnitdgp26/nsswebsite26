...
export function ContactSection() {
  return (
    <section
      ...
      aria-label="Contact NSS NIT Durgapur"
    >
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {contactContent.cards.map((card) => {
            ...
            return (
              <div
                key={card.key}
                className="flex flex-col rounded-2xl border bg-card p-6 transition-shadow hover:shadow-medium"
              >
        ...
        <div className="mt-8 overflow-hidden rounded-2xl border bg-muted">
          <div className="flex h-64 items-center justify-center sm:h-80">
            <div className="text-center">
              <MapPin className="mx-auto mb-3 h-8 w-8 text-primary/50" />

              <p className="text-sm text-muted-foreground">
                {contactContent.map.label}
              </p>

              <a
                href={contactContent.map.href}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-2 inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline"
              >
                {contactContent.map.action}
